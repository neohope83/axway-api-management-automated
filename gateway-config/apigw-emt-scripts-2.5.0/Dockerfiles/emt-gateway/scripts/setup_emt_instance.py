...
from com.axway.gw.es.yaml.model import ShortPathPK
...
PS_YAML_STORE_CONNECTION_FEATURES = StoreFeatures.builder() \
                                                 .enableAllowTypeUpgrade() \
                                                 .disableUpdateCheckCardinality() \
                                                 .enableValidationAllowInvalidRef() \
                                                 .disableValidationFailFast() \
                                                 .enableAllowUnresolvedAbsoluteFiles() \
                                                 .build().asProperties();
...
def _getProductAndYamlVersion():
    try:
        ...
        yamlES.connect("yaml:file:" + options.yamlConfig, PS_YAML_STORE_CONNECTION_FEATURES)
...
def _checkFeaturesUsed():
    ...
    # Check PortalConfiguration to see if API Manager is used
    if os.path.exists(options.yamlConfig) or options.factoryYaml :
        return checkYamlPortalConfiguration(configDir, False)
    else:
        return checkFedPortalConfiguration(configDir, False)

def checkYamlPortalConfiguration(config, apiManagerUsed = False):
    yamlES = YamlEntityStore()
    yamlES.connect("yaml:file:" + config, PS_YAML_STORE_CONNECTION_FEATURES)
    pc = ShortPathPK("/Server Settings/Portal Config")
    isPortalConfigExists = yamlES.existsEntity(pc)
    if isPortalConfigExists:
        apiManagerUsed = True
        entity = yamlES.getEntity(pc)
        if entity is not None:
            value = entity.get("adminPassword")
            if not value:
...
def checkFedPortalConfiguration(config, apiManagerUsed = False):
    newArchive = DeploymentArchive(config)
    es = EntityStoreAPI.wrap(newArchive.getEntityStore(), passphrases[ES_PASSPHRASE])
    pc = es.get("/[PortalConfiguration]name=Portal Config")
    if pc is not None:
        apiManagerUsed = True;
        value = pc.get("adminPassword")
        if not value:
            _fail("API Manager is configured but the API administrator has no default password.\n"
